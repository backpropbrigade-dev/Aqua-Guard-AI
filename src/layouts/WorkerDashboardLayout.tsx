...
const SIDEBAR_KEY = "aquaguard_worker_sidebar_collapsed";

function titleFromPath(pathname: string): string {
  if (pathname.includes("/missions")) return "My missions";
  if (pathname.includes("/profile")) return "Profile";
  if (pathname === "/dashboard/worker" || pathname === "/dashboard/worker/") return "Overview";
  ...
  return (
    <div className="min-h-screen bg-background font-body text-foreground flex">
      <aside className="hidden md:flex h-screen sticky top-0 z-30">
        <WorkerSidebar collapsed={collapsed} onToggleCollapse={toggleCollapse} />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        <header className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b border-border/60 glass px-4 shadow-sm md:px-6">
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent side="left" className="w-[280px] p-0 border-border/60">
              ...
              <WorkerSidebar collapsed={false} onToggleCollapse={() => {}} forceExpanded onNavigate={() => setMobileOpen(false)} />
            </SheetContent>
          </Sheet>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground truncate hidden sm:block">
              Field operations{session.identifier ? ` · ${session.identifier.split("@")[0]}` : ""}
            </p>
          </div>
        ...
        <main className="flex-1 p-4 md:p-6 lg:p-8 bg-gradient-to-b from-background via-background to-amber-500/[0.06] dark:to-amber-400/[0.08]">
          <Outlet />
        </main>
      </div>
    </div>
